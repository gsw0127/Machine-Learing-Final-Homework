...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if axelera:
            ...
            if not self.args.int8:
                ...
                self.args.int8 = True
            if model.task not in {"visdrone_detect"}:
        ...
        if imx:
            if not self.args.int8:
                ...
                self.args.int8 = True
            if not self.args.nms and model.task in {"visdrone_detect", "pose", "segment"}:
                LOGGER.warning("IMX export requires nms=True, setting nms=True.")
                self.args.nms = True
            if model.task not in {"visdrone_detect", "pose", "classify", "segment"}:
                raise ValueError(
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        if self.model.task == "classify":
            ...
            model = self.model
        elif self.model.task == "visdrone_detect":
        ...
        if self.args.nms and self.model.task == "visdrone_detect":
