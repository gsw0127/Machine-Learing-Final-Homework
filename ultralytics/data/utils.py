...
class HUBDatasetStats:
    ...
    Args:
        ...
        task (str): Dataset task. Options are 'visdrone_detect', 'segment', 'pose', 'classify'.
    ...
    Examples:
        ...
        >>> stats = HUBDatasetStats("path/to/coco8.zip", task="visdrone_detect")  # visdrone_detect dataset
    ...
    def __init__(self, path: str = "coco8.yaml", task: str = "visdrone_detect", autodownload: bool = False):
        """Initialize class."""
        path = Path(path).resolve()
        LOGGER.info(f"Starting HUB dataset checks for {path}....")

        self.task = task  # visdrone_detect, segment, pose, classify, obb
        if self.task == "classify":
            unzip_dir = unzip_file(path)
            data = check_cls_dataset(unzip_dir)
            data["path"] = unzip_dir
        else:  # visdrone_detect, segment, pose, obb
