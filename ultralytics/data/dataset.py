...
class YOLODataset(BaseDataset):
    ...
    Examples:
        >>> dataset = YOLODataset(img_path="path/to/images", data={"names": {0: "person"}}, task="visdrone_detect")
        >>> dataset.get_labels()
    """

    def __init__(self, *args, data: dict | None = None, task: str = "visdrone_detect", **kwargs):
        """Initialize the YOLODataset.

        Args:
            data (dict, optional): Dataset configuration dictionary.
            task (str): Task type, one of 'visdrone_detect', 'segment', 'pose', or 'obb'.
    ...
    def get_labels(self) -> list[dict]:
        ...
        if len_segments and len_boxes != len_segments:
            LOGGER.warning(
                ...
                "To avoid this please supply either a visdrone_detect or segment dataset, not a visdrone_detect-segment mixed dataset."
            )
            for lb in labels:
                lb["segments"] = []
        if len_cls == 0:
...
class GroundingDataset(YOLODataset):
    ...
    Examples:
        >>> dataset = GroundingDataset(img_path="path/to/images", json_file="annotations.json", task="visdrone_detect")
        >>> len(dataset)  # Number of valid images with annotations
    """

    def __init__(self, *args, task: str = "visdrone_detect", json_file: str = "", max_samples: int = 80, **kwargs):
        """Initialize a GroundingDataset for object detection.

        Args:
            json_file (str): Path to the JSON file containing annotations.
            task (str): Must be 'visdrone_detect' or 'segment' for GroundingDataset.
