...
TASKS = frozenset({"visdrone_detect", "segment", "classify", "pose", "obb"})
TASK2DATA = {
    "visdrone_detect": "coco8.yaml",
...
TASK2METRIC = {
    "visdrone_detect": "metrics/mAP50-95(B)",
...
def get_save_dir(args: SimpleNamespace, name: str | None = None) -> Path:
    ...
    Examples:
        >>> from types import SimpleNamespace
        >>> args = SimpleNamespace(project="my_project", task="visdrone_detect", mode="train", exist_ok=True)
        >>> save_dir = get_save_dir(args)
        >>> print(save_dir)
        my_project/visdrone_detect/train
    """
...
def entrypoint(debug: str = "") -> None:
    ...
    if task:
        if task not in TASKS:
            if task == "track":
                LOGGER.warning(
                    f"invalid 'task=track', setting 'task=visdrone_detect' and 'mode=track'. Valid tasks are {list(TASKS)}.\n{CLI_HELP_MSG}."
                )
                task, mode = "visdrone_detect", "track"
            else:
