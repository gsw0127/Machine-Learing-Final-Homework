...
def guess_model_task(model):
    ...
    Returns:
        (str): Task of the model ('visdrone_detect', 'segment', 'classify', 'pose', 'obb').
    """
    ...
    def cfg2task(cfg):
        ...
        if m in {"classify", "classifier", "cls", "fc"}:
            return "classify"
        if "visdrone_detect" in m:
            return "visdrone_detect"
        if "segment" in m:
            return "segment"
        if m == "pose":
            return "pose"
    ...
    if isinstance(model, (str, Path)):
        ...
        if "-seg" in model.stem or "segment" in model.parts:
            ...
            return "obb"
        elif "visdrone_detect" in model.parts:
            return "visdrone_detect"

    # Unable to determine task from model
    LOGGER.warning(
        "Unable to automatically guess model task, assuming 'task=visdrone_detect'. "
        "Explicitly define task for your model, i.e. 'task=visdrone_detect', 'segment', 'classify','pose' or 'obb'."
    )
    return "visdrone_detect"  # assume visdrone_detect
