...
def benchmark(
    ...
    **kwargs,
    ...
    data = data or TASK2DATA[model.task]  # task to dataset, i.e. coco8.yaml for task=visdrone_detect
    key = TASK2METRIC[model.task]  # task to metric, i.e. metrics/mAP50-95(B) for task=visdrone_detect
    ...
    for name, format, suffix, cpu, gpu, _ in zip(*export_formats().values()):
        ...
        try:
            ...
            if format == "imx":
                ...
                assert model.task in {"visdrone_detect", "classify", "pose"}, (
