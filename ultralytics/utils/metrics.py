...
class ConfusionMatrix(DataExportMixin):
    ...
    Attributes:
        task (str): The type of task, either 'visdrone_detect' or 'classify'.
    ...
    def __init__(self, names: dict[int, str] = [], task: str = "visdrone_detect", save_matches: bool = False):
        """Initialize a ConfusionMatrix instance.

        Args:
            names (dict[int, str], optional): Names of classes, used as labels on the plot.
            task (str, optional): Type of task, either 'visdrone_detect' or 'classify'.
    ...
    def tp_fp(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        return (tp, fp) if self.task == "classify" else (tp[:-1], fp[:-1])  # remove background class if task=visdrone_detect
...
class DetMetrics(SimpleClass, DataExportMixin):
    ...
    Attributes:
        ...
        task (str): The task type, set to 'visdrone_detect'.
